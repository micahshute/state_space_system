...
% See full_order_observer_simulation.slx
% Plot out states vs observed states. Can set
...
control_eigs = [-1+i, -1-i, -3];
observer_eigs = [-5,-5,-5];

co_sys = sys.to_full_observer_controller(observer_eigs, control_eigs);
[y1,t1] = co_sys.step;
hold on;
plot(t1,y1)
legend('OL', 'CL');

sys.simulate_full_observer_controller(observer_eigs, control_eigs, [1,-1,1]);
% Same output!

%% Reduced Observer Controller Example
hold off; close all; clear; clc;
a = [0,1,0;0,0,1;-4,-4,-1];
b = [0;0;1];
c = [1,1,1];
d = 0;
o_eigs = [-5,-5];
c_eigs = [-1, -0.5 + 1i*sqrt(3)/2, -0.5 - 1i*sqrt(3)/2];
x0=[1,-1,1];
sys = System(a,b,c,d);
rsys = sys.to_reduced_observer_controller(o_eigs, c_eigs);
rsys.display_ss
[y,t] = rsys.step;
plot(t,y);
title('Using to reduced observer controller')

sys.simulate_reduced_observer_controller(o_eigs, c_eigs, [0,0,0]);
rsys.display_ss

ceigs = [c_eigs, -2];
csys = sys.reduced_compensator(ceigs, o_eigs);
eigs(rsys.a)
eigs(csys.a)
t = linspace(0,20, 2000);
stepin = ones(size(t));
[y,t] = csys.simulate(stepin, t);
figure;
plot(t,y)

%% Use SS Tracking on Full Order Observer-Controller
hold off; close all; clear;clc;
a = [0,1;-1,-2];
b = [0;1];
c = [1,0];
...
opoles = [-10,-10];
sys = System(a,b,c,d);
csys = sys.full_compensator(cpoles, opoles);
csys.display_ss;
[y,t] = csys.step;
plot(t,y);
...
%Order of states: x1, x2, zeta, xhat1, xhat2
x0 = [-5;6;0;0;0];
csys = csys.set_x0(x0);
t= linspace(0,10, 1000);
step_input = ones(size(t));
[y,t,x] = csys.simulate(step_input, t);
figure;
plot(t,y)
hold on;
plot(t,x(:,4))
...
title('x2 vs x2hat');
sys = sys.set_x0([-5,6]);
sys.simulate_full_compensator(cpoles, opoles);
eigs(csys.a)
% Same output as switching and simulating
%% SS Tracking on Reduced Order Observer-Controller
hold off; close all; clear; clc;
a = [0,1,0;0,0,1;-10,-17,-8];
b = [0;0;1];
c = [1,1,1];
d = 0;
sys = System(a,b,c,d);
alphas = [1 4.2 13.6 21.6 16];
ceigs = roots(alphas);
oeigs = [-10;-10];
teigs = [-1, ceigs(3:4)']
rosys = sys.reduced_compensator(ceigs, oeigs);
tosys = sys.to_reduced_observer_controller(oeigs, teigs);
rosys.display_ss
t= linspace(0,20, 1000);
step_input = ones(size(t));
[y,t] = rosys.simulate(step_input, t);
plot(t,y)
ceigs
eigs(rosys.a)
[y,t] = sys.step;
...
plot(t,y)
sys.poles
sys.simulate_reduced_compensator(ceigs,oeigs)
...
csys = sys.to_reduced_observer_controller(oeigs, peigs);
[t,l,e,pt,qt,j,m,n] = sys.get_all_reduced_observer_info(oeigs)
